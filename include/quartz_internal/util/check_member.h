...
template<typename C, typename Ret, typename... Args>
struct has_expectation<C, Ret(Args...)> {
private:
  template<typename T>
  static constexpr auto check(T*)
  -> typename
  std::is_same<
      decltype( std::declval<T>().expectation( std::declval<Args>()... ) ),
      Ret    // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  >::type {
    return nullptr;
  }
...
private:
  ...
  -> typename
  std::is_same<
      decltype( std::declval<T>().normalise( std::declval<Args>()... ) ),
      Ret    // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  >::type {
    return nullptr;
  }
