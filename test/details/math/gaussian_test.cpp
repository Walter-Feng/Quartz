...
TEST_CASE("Gaussian Value") {
  SECTION("One Dimension") {
    ...
    for (arma::uword i = 0; i < grid.n_elem; i++) {
      CHECK(std::real(gaussian1.at(arma::vec{grid(i)})) == std::exp(-0.5 * grid(i) * grid(i)));
    }
...
TEST_CASE("Gaussian Integral") {
  SECTION("Real space") {
    ...
    CHECK(std::abs((test_multi * test_multi).integral() - wigner_transform_multi.integral()) < 1e-14);
  }
